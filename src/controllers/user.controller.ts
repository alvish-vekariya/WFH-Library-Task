...
export class userController{
    ...
    async register(req: Request, res: Response){
        ...
        if(!username || !password) return res.status(STATUS.NOT_FOUND).send(ERRORS.PARAMETERS_NOT_FOUND);
        try{
            const resp = await this.userService.register(username, password) as eventInterface;
    ...
    async login(req: Request, res: Response){
        ...
        if(!username || !password) return res.status(STATUS.NOT_FOUND).send(ERRORS.PARAMETERS_NOT_FOUND);
        try{
            const resp = await this.userService.login(username, password) as eventInterface;
            return res.status(resp.statusCode).send(resp);
        }catch(err : any){
            return res.status(err.data.statusCode).send(err.data);
        }
    ...
    async logout(req: Request, res: Response){
        ...
        if(!username) return res.status(STATUS.NOT_FOUND).send(ERRORS.PARAMETERS_NOT_FOUND);
        try{
