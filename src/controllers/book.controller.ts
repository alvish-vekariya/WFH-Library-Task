...
export class bookController {
    ...
    async addBook(req: Request, res: Response) {
        ...
        if(!title || !author || !category || !isbn || !description || !price || !token) return res.status(STATUS.NOT_FOUND).json(ERRORS.PARAMETERS_NOT_FOUND);
    ...
    async updateBook(req: Request, res: Response) {
        ...
        if(!title || !author || !category || !isbn || !description || !price || !bookID || !token) return res.status(STATUS.NOT_FOUND).json(ERRORS.PARAMETERS_NOT_FOUND);

        
        try{
            return res.send(await this.bookService.updateBook(bookID, title, author, category, isbn, description, price, token_data.userID));
        }catch(err : any){
            if(err.code && err.code === 11000){
                return res.status(STATUS.CONFLICT).send(ERRORS.DUPLICATION_ERROR);
            }else{
    ...
    async getBook(req: Request, res: Response) {
        ...
        if(!bookID) return res.status(STATUS.NOT_FOUND).json(ERRORS.PARAMETERS_NOT_FOUND);

        try{
            return res.status(STATUS.SUCCESS).send(await this.bookService.getBook(bookID));
        }catch(err: any){
            return res.status(err.data.statusCode).send(err.data);
        }
    }
